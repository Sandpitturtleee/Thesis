...
Na rys.~\ref{fig:standard_naive_mean} przedstawiono średnią liczbę operacji wykonywanych przez naiwną wersję algorytmu. Dla grafu o średniej gęstości, grafu gęstego oraz przypadku szczególnego widoczny jest ten sam przebieg. Wynika to ze sposobu zliczania operacji w tej implementacji. Podczas pojedynczej iteracji algorytmu sprawdzane jest wszystkie \(n\) wierzchołków, niezależnie od liczby krawędzi oraz ich wag. Jeżeli graf jest spójny, algorytm wykonuje \(n\) takich iteracji. Liczba zliczonych operacji jest zatem równa
...
\noindent
W tym miejscu widoczna jest cecha implementacji \textit{lazy heap}. Poprzednie elementy
